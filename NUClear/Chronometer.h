...
template <typename TChild>
NUClear::Chronometer<TChild>::Chronometer() : 
    chronometer(this) {
}

template <typename TChild>
NUClear::Chronometer<TChild>::Implementation::Implementation(Chronometer* parent) :
    parent(parent),
    execute(true) {
}

template <typename TChild>
NUClear::Chronometer<TChild>::Implementation::~Implementation() {
    execute = false;
}

template <typename TChild>
template <int ticks, class period>
void NUClear::Chronometer<TChild>::Implementation::add() {
    
    // Check if we have not already loaded this type in
    if(loaded.find(typeid(NUClear::Every<ticks, period>)) == std::end(loaded)) {
        
        // Flag this type as loaded
        loaded.insert(typeid(NUClear::Every<ticks, period>));
        
        std::function<void ()> emit = [this](){
            static_cast<TChild*>(parent)->emit(new Every<ticks, period>());
        };
...
template <typename TChild>
void NUClear::Chronometer<TChild>::Implementation::run() {
