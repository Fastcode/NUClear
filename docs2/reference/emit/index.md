...
Every `emit` call takes a `std::shared_ptr` or `std::unique_ptr` to the data being emitted. The scope template parameter determines the distribution strategy. When no scope is specified, `Scope::LOCAL` is used.
...
| `Scope::INITIALISE` | Queues data to be emitted during system startup | [Initialise](initialise.md) |
